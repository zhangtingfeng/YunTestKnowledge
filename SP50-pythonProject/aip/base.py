...
class AipBase(object):
    ...
    def _request(self, url, data, HUAWEI, headers=None, ):
        """
            self._request('', {})
        """
        try:

            if (HUAWEI == "HUAWEI_X-Subject-Token" or headers != None):
            ...
            if (HUAWEI == "HUAWEI_X-Subject-Token"):
                obj = (response.headers["X-Subject-Token"])
            else:
                obj = json.loads(response.content.decode('utf8', 'ignore').encode('utf8')) or {}
