...
public class DynamicOperateExcelUtils {
    ...
    public void InserImg(int rowNum, int colNum, String strFileNamevalue) throws IOException {
        //将图片读到BufferedImage
        ByteArrayOutputStream byteArrayOut = new ByteArrayOutputStream();

        BufferedImage bufferImg = ImageIO.read(new File(strFileNamevalue));
        // 将图片写入流中
        ImageIO.write(bufferImg, "png", byteArrayOut);
        int pictureIdx = workBook.addPicture(byteArrayOut
                .toByteArray(), XSSFWorkbook.PICTURE_TYPE_PNG);
        //图片一导出到单元格B2中 前四个参数是图片的边距
        // 后四个参数的第一个是X轴的起始位置、第三个是X轴的结束位置、
        // 第二个是Y轴的起始位置、第四个是Y轴的结束位置
        XSSFClientAnchor anchor = new XSSFClientAnchor(0, 0, 0, 0,
                (short) colNum, rowNum, (short) colNum+1, rowNum+1);
        // 插入图片
        XSSFDrawing patriarch = (XSSFDrawing) sheet.createDrawingPatriarch();
        patriarch.createPicture(anchor, pictureIdx);
        ///FileOutputStream stream = cn.eggsoft.sp34pricereport.FileUtils.openOutputStream(new File(strFileNamevalue));
    ...
    public void updateRows(int startRowIndex, List<Map<String, Object>> datas, String[] keys) {
        // 插入行
        sheet.shiftRows(startRowIndex, startRowIndex + datas.size(), datas.size(), true, false);// 第1个参数是指要开始插入的行，第2个参数是结尾行数,第三个参数表示动态添加的行数
        // 向插入的行中动态的填充数据
        for (int i = 0; i < datas.size(); i++) {
